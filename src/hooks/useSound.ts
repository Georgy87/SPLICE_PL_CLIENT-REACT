...
export const useSound = () => {
  ...
  const playTrack = (index: number, typeElement: string) => {
    ...
    const url = typeElement === 'packs' ? `${packs[index]?.audio}` : `${samples[index]?.audio}`;
  ...
  const changeCurrentTime = (e: React.ChangeEvent<HTMLInputElement>) => {
    playerState.audioPlayer.currentTime = (playerState.audioPlayer.duration / 100) * Number(e.target.value);
  };
