...
export class CanvasChartService implements ICanvasChart {
  ...
  drawingChart(canvas: HTMLCanvasElement | null, coordsData: MountType, width: number, padding: number) {
    this.width = width;
  ...
  drawingElements(
    ...
    this.paint(ctx, coordsData, mountNames, coordsXY, proxy, canvas, requestId, mousemove, mouseleave);
  ...
  paint(
    ...
    const coords: number[][] = coordsXY.map(({ x, y }, i) => {
